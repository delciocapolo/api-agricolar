...
export class DatabaseConnectionPOST {
    ...
    async createCostumer(consumidor, localizacao) {
        try {
    ...
    async createWishList({ id_consumidor, id_produto }) {
        try {
    ...
    async createFarm({ id_fazendeiro, fazenda }) {
        try {
...
export class DatabaseConnectionGET {
    ...
    async getSoldProduct(limit, id_sold_product) {
        try {
            if (id_sold_product) {
    ...
    async getClientsFromFarm(id_fazenda) {
        try {
            const status = await this.farmExists(id_fazenda);
    ...
    async getStatisticFromFarm(id_fazenda) {
        try {
            const status = await this.farmExists(id_fazenda);
            if (status?.status !== undefined && !status.status) {
                console.error(status.message);
