...
try {
    ...
    app.use("/v1/set", expressMiddleware(SetServer, {
        context: async ({ req, res }) => ({
            token: "",
        }),
    }));
    await new Promise((resolve) => httpServer.listen({ port: SET_SERVER_PORT }, resolve));
    log(`📬 SERVER IS RUNNIG AT: http://localhost:${SET_SERVER_PORT}/v1/set`);
}
catch (error) {
    console.error(error);
}
//# sourceMappingURL=serverSet.mjs.map